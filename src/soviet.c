...
WINDOW *wCCCP;
WINDOW *wsoviet;
coordinates s_snake[9000];
coordinates s_food[9000];
...
void soviet() 
{
	...
	CCCP();
	//soviet window
	wsoviet=newwin(lines,cols,0,20);
	keypad(wsoviet,TRUE);
	...
	s_food[nfood]=randomfood(lines, cols);
	//Main loop
	while (s_cicle) 
	{
		...
		s_food[nfood]=randomfood(lines, cols); //each cicle gets new food
		// Look not to die :D
		s_cicle=loopcollision (s_snake, lines, cols, s_len); //collisions
		if (s_len<=0) 
		{ //end of the snake
			s_cicle=FALSE; 
		}
